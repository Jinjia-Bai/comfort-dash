...
def t_hr_pmv(inputs: dict = None, model: str = "iso"):
    ...
    # 遍历PMV上下限，并对不同的rh进行计算
    for pmv_limit in pmv_limits:
        for rh in np.arange(10, 110, 10):  # rh每次增加10%
            psy_data_rh = psy_ta_rh(current_tdb, rh)

            # 使用optimize.brentq计算在给定rh下的温度
            def function(x):
                return (
                        pmv(
                            x,
            ...
            temp = optimize.brentq(function, 10, 40)  # 寻找温度解
            results.append(
                {
                    "rh": rh,
                    "hr": psy_data_rh['hr'] * 1000,  # 转化为g/kg
                    "temp": temp,
                    "pmv_limit": pmv_limit,
                }
            )

    df = pd.DataFrame(results)  # 结果转换为DataFrame
    ...
    for rh in np.arange(10, 110, 10):
        temp_range = np.arange(10, 40, 1)  # 温度范围
        hr_values = [psy_ta_rh(t, rh)["hr"] * 1000 for t in temp_range]  # 计算湿度比
        ax.plot(temp_range, hr_values, color="grey", linestyle="--")  # 移除label避免图例
    ...
    ax.scatter(
        current_tdb,
        psy_data["hr"] * 1000,  # 湿度比
        color="red", edgecolor="black", s=100  # 不需要label避免图例
    )
